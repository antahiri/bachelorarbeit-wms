...
Die lokale Ebene in Kapitel~\ref{sec:lokal} vergleicht die fünf
...
Die Workflows, die Benchmark-Skripte und die Instrumentierung sind
dieselben wie auf der lokalen Ebene, ebenso die Definition der
Messgrößen aus Abschnitt~\ref{sec:methodik:messgroessen}. Die
Ausführungsbedingungen unterscheiden sich jedoch, weshalb die
Ergebnisse beider Ebenen getrennt betrachtet und nicht unmittelbar
...
Referenzmessung ohne Workflow-Management-System und die fünf beziehungsweise drei Wiederholungen je Konfiguration aus denen der Median gebildet wurde. 
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Differenzen zwischen exklusivem und geteiltem Knoten je
    Konfiguration, jeweils Median. Werte nahe null bedeuten, dass
    die exklusive Reservierung das Ergebnis nicht wesentlich
    verändert.}
...
zwischen 1,6 und 3,9 Sekunden und wechseln untereinander die
...
StreamFlow bei 5,4 und 6,8 Sekunden und Nextflow bei 5,0 und 4,6
...
einem stimmigen Bild. AiiDA weist sowohl beim Overhead als auch bei den
Koordinationsgrößen die höchsten Werte auf. Bereits im
Pipeline-Muster summieren sich die drei Übergangslatenzen von
...
etwa eine Sekunde erreicht. Bei StreamFlow, Nextflow und Merlin
bleiben die Koordinationszeiten dagegen deutlich kleiner, was sich
auch in ihren niedrigeren Overhead-Werten widerspiegelt. Wie bereits auf der lokalen Ebene sind die beobachteten Unterschiede damit auch auf dem HPC-System überwiegend der Koordination und den Übergaben durch die Workflow-Management-Systeme zuzuordnen.

\section{Einfluss der AiiDA-Konfiguration}
\label{sec:mogon:konfigstudie}

AiiDA weist in der dedizierten Messung höhere Overhead- und
Koordinationswerte auf als die drei übrigen Systeme. Zugleich ist
...
in allen zwölf Fällen von 12,6 bis 29,9 Sekunden auf 5,7 bis
9,9 Sekunden. Je nach Messkonfiguration entspricht dies einer
Verringerung um den Faktor 2 bis 3,7. Die
PostgreSQL-Konfiguration liegt mit 8,4 bis 15,3 Sekunden bei
...
Datenhaltung zurückzuführen. Zugleich bleibt AiiDA auch in der
schnellsten der drei untersuchten Konfigurationen mit 5,7 bis
9,9 Sekunden Overhead über dem Bereich der drei übrigen Systeme.
Der in Abschnitt~\ref{sec:mogon:ergebnisse} beobachtete Abstand
bleibt damit auch bei veränderter AiiDA-Konfiguration bestehen,
während die absolute Höhe des Overheads deutlich von der
...
in einem engen Bereich, während AiiDA höhere Overhead- und
Koordinationswerte aufweist. Wie auf der lokalen Ebene stehen die
Unterschiede dabei vor allem mit den Übergängen zwischen den
Workflow-Schritten und dem Start paralleler Verarbeitungsinstanzen
in Zusammenhang. Die ergänzende Untersuchung von AiiDA zeigt
darüber hinaus, dass die Höhe dieser Werte deutlich von
der gewählten AiiDA-Konfiguration und insbesondere vom Ablageort der
Datenhaltung beeinflusst werden kann.

Die HPC-Untersuchung bestätigt damit einerseits mehrere bereits auf
der lokalen Ebene beobachtete Unterschiede zwischen den Systemen,
zeigt andererseits aber zusätzliche Aspekte, die erst in
einer Clusterumgebung sichtbar werden. Dazu gehören die
Bereitstellung unter Nutzerrechten, die Einbindung des
Ressourcenverwalters, clusterbedingte Zeitanteile und der Einfluss
der Ausführungsumgebung auf einzelne Systeme. Diese Befunde bilden
zusammen mit den Ergebnissen der lokalen Untersuchung die Grundlage
für die abschließende Bewertung der Workflow-Management-Systeme.