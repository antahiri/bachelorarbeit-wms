...
Untersuchungsdesign fest, begründet die Auswahl der fünf Systeme,
führt die qualitativen Vergleichsaspekte und die quantitativen
...
Rechner umgesetzt und vermessen, diese Ebene bildet die gemeinsame
Vergleichsbasis. Auf der zweiten Ebene wird untersucht, wie sich die
Systeme auf einem Slurm-basierten HPC-System bereitstellen und
...
Berücksichtigt wurden drei Merkmale. Das erste ist der
Anwendungskontext, in dem die Systeme entstanden sind, er wurde oben
je System benannt und reicht von der Bioinformatik bis zur
Forschungsdatenverwaltung mit durchgängiger Provenance. Auch Suter
...
Ausführung an eigenständige Komponenten. Wie diese Merkmale in den
einzelnen Systemen ausgeprägt sind, zeigt die Modellierung in
...
sieben Aspekten: dem Ausdruck des Musters, dem Modellierungs- und
...
\emph{Ausführung und Steuerung:} Dieser Aspekt betrachtet, welche Komponente einen beschriebenen Workflow ausführt und wodurch der Beginn eines einzelnen Schritts ausgelöst wird. Damit bildet er die Verbindung zwischen der Workflow-Beschreibung und dem beobachtbaren Ablauf eines Workflows, da sich daraus ergibt, wann und auf welchem Weg einzelne Schritte ausgeführt werden. Untersucht wird der Aspekt anhand der Komponenten und Mechanismen, die die Ausführung der Workflow-Schritte übernehmen und ihren Ablauf bestimmen.
...
Größen, die zugehörige Instrumentierung beschreibt
Abschnitt~\ref{sec:methodik:instrumentierung}.
...
Koordinationsvorgänge innerhalb eines Laufs gesondert. Makespan und
Overhead knüpfen an bestehende Arbeiten zur Leistungsanalyse von
Workflow-Läufen an. Chen und Deelman analysieren den Zeitbedarf
eines Workflow-Laufs anhand des Makespan und verschiedener
Overhead-Anteile des Workflow-Management-Systems und der
Ausführungsumgebung~\cite{chen2011overhead}. Die beiden
Koordinationsgrößen werden für diese Arbeit eingeführt. Sie
bestimmen den zeitlichen Abstand zwischen zwei aufeinanderfolgenden
Rechenschritten beziehungsweise die Streuung der Startzeitpunkte
...
wissenschaftlicher Workflows beschrieben werden. Suter et al.
nennen Datenverarbeitungs-Pipelines und
Fan-out/Fan-in-Ausführungsmuster als die einfachen Formen, aus
denen sich komplexere Workflows
zusammensetzen~\cite[S.~4]{suter2026terminology}.
...
Grundkomponenten, die Bharathi et al. aus fünf realen
wissenschaftlichen Anwendungen abstrahieren und ihrerseits als
Grundlage für Benchmark-Workflows nutzen, darunter sequenzielle
Verarbeitungsketten, die Zerlegung von Datensätzen in Teilstücke
und deren anschließende
Zusammenführung~\cite[S.~3]{bharathi2008characterization}. Die
konkrete Ausgestaltung der einzelnen Schritte stellt eine
...
Aus den Mustern, Rechenlasten und Teilstückzahlen ergeben sich der
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[res, below=7mm of agg] (o1) {Gesamtlaufzeit: Median von $T_{\mathrm{Sys}}$};
    \node[res, below=4mm of o1] (o2) {Overhead: Median von $T_{\mathrm{Sys}}$ minus Median von $T_{\mathrm{Ref}}$};
    \node[res, below=4mm of o2] (o3) {Median von $L_i$ und von $S$};
    \draw[arr] (cfg) -- (r1);
    \draw[arr] (r1) -- (r2);
    \draw[arr] (r2) -- (r3);
...
Der Referenzlauf verwendet dieselben Benchmark-Skripte und dieselben
fachlichen Ein- und Ausgaben wie der zugehörige Systemlauf und führt
sie in der Reihenfolge des jeweiligen Musters aus. Das Steuerskript
startet die Skripte unmittelbar als eigene Prozesse. Beim Pipeline-Muster
laufen die vier Schritte nacheinander ab. Beim
Scatter-Gather-Muster folgen auf die Vorverarbeitung und die
Zerlegung so viele gleichzeitig gestartete Verarbeitungsprozesse,
wie Teilstücke vorgesehen sind, anschließend das Zusammenführen und
die Nachbereitung. Da die Eingabe über einen festen Startwert
erzeugt wird, verarbeiten Referenz- und Systemlauf dieselben Werte.
Der Referenzlauf bildet damit dieselbe Rechenlogik und dieselbe
Struktur ab wie der Systemlauf, jedoch ohne die Orchestrierung durch ein Workflow-Management-System.
...
Die äußere Messung erfasst die Gesamtlaufzeit eines Laufs.
Unmittelbar vor der Übergabe des Workflows an ein System und nach
dessen vollständigem Abschluss nimmt das Steuerskript je einen
Zeitstempel, die Zeitspanne dazwischen bildet die Gesamtlaufzeit
$T_{\mathrm{Sys}}$ eines Systemlaufs beziehungsweise
$T_{\mathrm{Ref}}$ des zugehörigen Referenzlaufs.

Der Beginn der Messung liegt bei allen Systemen unmittelbar vor dem
Startbefehl. Wie das Ende eines Laufs festgestellt wird, hängt vom
...
\begin{table}[H]
  ...
  \begin{tabular}{l l p{6.4cm}}
    ...
    \hline
    Nextflow
    & vor \texttt{nextflow run}
    & Rückkehr des blockierenden Aufrufs \\

    StreamFlow
    & vor \texttt{streamflow run}
    & Rückkehr des blockierenden Aufrufs \\

    Merlin
    & vor \texttt{merlin run}
    & \texttt{MERLIN\_STATUS.json} mit Status \texttt{FINISHED} und
      \texttt{MERLIN\_FINISHED} im Workspace des terminalen Schritts
      \texttt{postprocess}, Abfrage alle 20\,ms \\

    AiiDA
    & vor dem Einreichen der WorkChain
    & terminaler Zustand des obersten WorkChain-Prozessknotens,
      Abfrage alle 50\,ms \\

    Pegasus
    & vor \texttt{pegasus-plan}
    & erfolgreicher Workflow-Zustand über
      \texttt{pegasus-status -l}, Abfrage alle 500\,ms \\
    \hline
  \end{tabular}
\end{table}
...
Jeder Schritt nimmt unmittelbar nach dem Start und unmittelbar vor
dem Ende einen Zeitstempel und legt anschließend eine Timing-Datei
...
Sei $R$ die Zahl der Wiederholungen einer Konfiguration,
...
\[
  ...
  \qquad
  O = \operatorname{Median}_{r}\bigl(T_{\mathrm{Sys},r}\bigr)
      - \operatorname{Median}_{r}\bigl(T_{\mathrm{Ref},r}\bigr).
\]
Der Overhead wird damit entsprechend seiner Definition in
Abschnitt~\ref{sec:methodik:messgroessen} als Differenz der
Mediane beider Laufreihen bestimmt.
...
Das Auswertungsskript liest nach Abschluss aller Wiederholungen die
Laufzeitmessungen und sämtliche Timing-Dateien einer Konfiguration
ein. Anschließend prüft es die Vollständigkeit der erwarteten
Schritte, ordnet die Zeitstempel den jeweiligen Workflow-Schritten
zu, berechnet daraus die Messgrößen jeder Wiederholung und bildet
abschließend die Mediane.
