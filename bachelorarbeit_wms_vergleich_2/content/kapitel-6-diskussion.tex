...
einordnen~\cite{suter2026terminology}.
...
an der Workflow-Beschreibung. Die Modellierungsuntersuchung zeigt
...
Auf beiden Untersuchungsebenen zeigt sich dieselbe Anordnung der
Systeme. StreamFlow, Nextflow und Merlin liegen beim Overhead in
einem gemeinsamen Bereich, AiiDA darüber, und Pegasus in der
lokalen Untersuchung mit deutlichem Abstand an der Spitze. Dass
diese Anordnung auf einem einzelnen Rechner und auf einem
dedizierten Rechenknoten gleichermaßen auftritt, spricht dagegen,
sie der jeweiligen Ausführungsumgebung zuzuschreiben. Ein
...
Die beiden Koordinationsgrößen zeigen, an welchen Stellen ein
wesentlicher Teil dieser Unterschiede entsteht. Besonders deutlich ist dies bei Pegasus und AiiDA. Bei Pegasus fällt die mediane Übergangslatenz von rund 20 Sekunden mit dem höchsten gemessenen Overhead zusammen. Bei AiiDA fallen die Übergangslatenzen mit ein bis zwei Sekunden geringer aus, liegen aber auf beiden Ebenen deutlich über denen der drei übrigen Systeme und erklären bereits im Pipeline-Muster einen wesentlichen Teil des Overheads. Die Ergebnisse sprechen damit dafür, dass die höheren Overhead-Werte von Pegasus und AiiDA zu einem erheblichen Teil mit der Koordination und Übergabe
zwischen aufeinanderfolgenden Workflow-Schritten zusammenhängen.

Die Startspreizung ist davon getrennt zu betrachten. Sie beschreibt, wie gut das System Parallelität koordinieren kann. Merlin zeigt das deutlich: Lokal liegt seine
...
nicht zwangsläufig zu einem höheren Overhead. Beide
Koordinationsgrößen ergänzen die Messung der Gesamtlaufzeit, indem
sie unterschiedliche Stellen sichtbar machen, an denen ein
Workflow-Management-System zusätzliche Zeit beansprucht.
...
104 Millisekunden lag. Eine Ende-zu-Ende-Messung im regulären
Clusterbetrieb würde damit überwiegend die zum Messzeitpunkt
vorliegenden Clusterbedingungen abbilden und die Unterschiede
zwischen den Systemen überlagern. Für den eigentlichen
Systemvergleich war deshalb eine bereits bestehende
Slurm-Allokation erforderlich, innerhalb derer Warteschlange und
Jobstart nicht erneut für jeden Workflow-Schritt anfielen.
...
Aus den Ergebnissen ergibt sich kein System, das den übrigen in
allen Belangen überlegen wäre. Die Eignung hängt vielmehr davon ab,
welche Anforderungen hinsichtlich Modellierung, Laufzeitverhalten
und HPC-Betrieb im Vordergrund stehen.
...
Muster ließen sich kompakt über Prozesse und Kanäle abbilden, wobei
...
bewährte sich beim Wechsel auf MOGON: Die in CWL formulierte
Workflow-Beschreibung blieb unverändert, während lediglich die
Ausführungskonfiguration angepasst wurde. Zusammen mit den
niedrigsten gemessenen Übergangslatenzen eignet sich StreamFlow
damit insbesondere, wenn eine standardisierte Workflow-Beschreibung
und deren Übertragung zwischen unterschiedlichen
Ausführungsumgebungen im Vordergrund stehen. Einschränkend war die
...
Worker~\cite{peterson2022merlin}. Dieses Modell passt insbesondere
zum Scatter-Gather-Muster mit mehreren gleichartigen parallelen
Verarbeitungsschritten. Auf MOGON erwies sich außerdem die
gemeinsame Worker-Allokation als charakteristisch: Die einzelnen
Workflow-Schritte werden innerhalb bereits bereitgestellter
Ressourcen ausgeführt, sodass Warte- und Startzeiten des
Ressourcenverwalters nicht je Schritt erneut entstehen. Merlin
bietet sich damit insbesondere für Workflows mit vielen
gleichartigen Rechenaufgaben innerhalb einer gemeinsamen
HPC-Allokation an, erfordert dafür jedoch zusätzliche Komponenten
für Aufgabenvermittlung und Worker. Die von Merlin angestrebte
Skalierung auf sehr große Aufgabenzahlen wurde in dieser Arbeit
nicht untersucht, da das Scatter-Gather-Muster höchstens vier
parallele Verarbeitungsinstanzen umfasste.
...
Ausführung in einem Provenancegraphen
...
gemessenen Zeiten. AiiDA bietet sich damit insbesondere für
Anwendungsfälle an, in denen die dauerhafte Nachvollziehbarkeit von
Prozessen und Daten den zusätzlichen Koordinations- und
Betriebsaufwand rechtfertigt, wobei die absolute Höhe der
gemessenen Zeiten von der Konfiguration der Datenhaltung abhängt.

Pegasus trennt die abstrakte Beschreibung eines Workflows von
dessen Abbildung auf die konkrete
Ausführungsumgebung~\cite[S.~18--20]{deelman2015pegasus}. Dieser
Ansatz eignet sich insbesondere für Workflows, die auf wechselnden
oder verteilten Recheninfrastrukturen ausgeführt werden sollen. In
der lokalen Untersuchung stand dem der mit Abstand höchste Overhead
gegenüber, der bei kurzen Rechenschritten besonders stark ins
...
Bündeln mehrerer Aufgaben vor~\cite{pegasusdocs-optimizing}. Für
feingranulare Workflows ist dieser Ausführungsaufwand daher
besonders zu berücksichtigen. Die Eignung für das untersuchte
HPC-System konnte nicht bewertet werden, da sich Pegasus dort nicht bereitstellen ließ.

Systemübergreifend tritt damit ein weiteres Kriterium für die
Systemwahl hervor: die Größe der einzelnen Aufgaben im Zusammenspiel
mit dem Ausführungsmodell. Sind die Rechenschritte groß, verlieren
die Unterschiede in der Koordination an Gewicht, und die Wahl kann
sich an den übrigen Anforderungen ausrichten, etwa an der
Beschreibungsform, der Übertragbarkeit zwischen Umgebungen oder der
Nachvollziehbarkeit. Bei kleinen Rechenschritten rückt dagegen in
den Vordergrund, wie ein System seine Aufgaben ausführt. Wer jeden
Schritt einzeln an den Ressourcenverwalter übergibt, trägt dessen
Anteile je Job in voller Höhe, während ein Modell, das die Aufgaben
innerhalb bereits bereitgestellter Ressourcen abarbeitet oder
mehrere kurze Aufgaben zu größeren Einheiten bündelt, diese Anteile
nur einmal anfallen lässt. Die Eignung eines
Workflow-Management-Systems bemisst sich damit nicht allein an der
Struktur des Workflows, sondern ebenso an der Größe der Aufgaben
und an der Art, wie das System sie auf der Zielumgebung ausführt.
Die Grenzen dieser Aussagen betrachtet
...
Rechenlogik ausführen. Dadurch konnten Unterschiede zwischen den
Workflow-Management-Systemen unter kontrollierten Bedingungen
untersucht werden. Die verwendeten Rechenaufgaben bilden jedoch
...
Median ausgewertet. Die Verwendung des Medians reduziert den Einfluss einzelner abweichender Läufe, eine größere Zahl an Wiederholungen hätte jedoch eine genauere Beurteilung der Schwankungen zwischen den einzelnen Läufen ermöglicht.
